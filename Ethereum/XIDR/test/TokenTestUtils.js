...
var bigZero = web3.utils.toBN(0);
var bigHundred = web3.utils.toBN(100);
...
var delegateAccount = "0x1dF62f291b2E969fB0849d99D9Ce41e2F137006e"; // accounts[9]
...
var pauserAccountPrivateKey = "829e924fdf021ba3dbbc4225edfece9aca04b929d6e75613329ca6f1d31c0bb4"; // accounts[8]
var proxyOwnerAccountPrivateKey = "21d7212f3b4e5332fd465877b64926e3532653e2798a11255a46f533852dfe46"; // accounts[14]
...
function checkMinterConfiguredEvent(increaseMinterAllowanceEvent, minter, minterAllowedAmount) {
    assert.equal(increaseMinterAllowanceEvent.logs[0].event, 'MinterConfigured')
    assert.equal(increaseMinterAllowanceEvent.logs[0].args.minter, minter)
    assert.equal(increaseMinterAllowanceEvent.logs[0].args.minterAllowedAmount, minterAllowedAmount)
}
...
function checkTransferEventsWithFee(transferEvent, from, to, amount, feeAmount) {
    ...
    assert.equal(transferEvent.logs[1].args.amount, amount);
}

function checkTransferEvents(transferEvent, from, to, amount) {
    assert.equal(transferEvent.logs[0].event, 'Transfer');
    assert.equal(transferEvent.logs[0].args.from, from);
    assert.equal(transferEvent.logs[0].args.to, to);
    assert.equal(transferEvent.logs[0].args.amount, amount);
}

function checkApprovalEvent(approvalEvent, approver, spender, amount) {
    ...
    assert.equal(approvalEvent.logs[0].args.amount, amount);
}
...
function checkMintEvent(minting, to, amount, minter) {
    // MinterRemoved Event when minter allowance hits 0
    var decreaseMinterAllowanceLogIndex = minting.logs.length == 3 ? 0 : -1;
    var mintLogIndex = decreaseMinterAllowanceLogIndex + 1;
    var transferLogIndex = mintLogIndex + 1;

    if(decreaseMinterAllowanceLogIndex == 0) {
        assert.equal(minting.logs[decreaseMinterAllowanceLogIndex].event, 'MinterRemoved');
        assert.equal(minting.logs[decreaseMinterAllowanceLogIndex].args.oldMinter, minter);
    }

    // Mint Event
    assert.equal(minting.logs[mintLogIndex].event, 'Mint');
    assert.equal(minting.logs[mintLogIndex].args.minter, minter);
    assert.equal(minting.logs[mintLogIndex].args.to, to);
    assert.equal(minting.logs[mintLogIndex].args.amount, amount);

    // Transfer from 0 Event
    assert.equal(minting.logs[transferLogIndex].event, 'Transfer');
    assert.equal(minting.logs[transferLogIndex].args.from, 0);
    assert.equal(minting.logs[transferLogIndex].args.to, to);
    assert.equal(minting.logs[transferLogIndex].args.amount, amount);
...
function checkBurnEvents(burning, amount, burner) {
    ...
    assert.equal(burning.logs[1].args.amount, amount);
...
async function checkVariables(_tokens, _customVars) {
    ...
    for (n = 0; n < numTokens; n++) {
        ...
        var accounts = [arbitraryAccount, masterMinterAccount, minterAccount, pauserAccount, blacklisterAccount, tokenOwnerAccount, upgraderAccount, token.address];
        var balanceSum = bigZero;
        var x;
        for (x = 0; x < accounts.length; x++) {
            balanceSum = balanceSum.add(web3.utils.toBN(await token.balanceOf(accounts[x])));
        }
        var totalSupply = web3.utils.toBN(await token.totalSupply())
    ...
    return Q.all([
        ...
        var actualState = {
            ...
            'balances': {
                ...
                'tokenContractAccount': balancesTC
            },
            'allowance': {
                'arbitraryAccount': {
...
async function setMinter(token, minter, amount) {
    let oldMinterAllowance = await token.minterAllowance(minter);
    let update = await token.increaseMinterAllowance(minter, amount, { from: masterMinterAccount });
    assert.equal(update.logs[0].event, 'MinterConfigured');
    assert.equal(update.logs[0].args.minter, minter);
    // Need to use minterAllowance + amount because masterMinter gave minter 900 allowance in the prev revert call
    assert.equal(update.logs[0].args.minterAllowedAmount, oldMinterAllowance.toNumber() + amount);
    let minterAllowance = await token.minterAllowance(minter);
    assert.equal(minterAllowance, oldMinterAllowance.toNumber() + amount);
}
...
async function mintRaw(token, to, amount, minter) {
    ...
    /*  let totalSupply = await token.totalSupply();
      ...
      let minterAllowance = await token.minterAllowance(minter);
      assert.isTrue(web3.utils.toBN(initialMinterAllowance).sub(web3.utils.toBN(amount)).eq(web3.utils.toBN(minterAllowance)));*/
}
...
async function setLongDecimalFeesTransferWithFees(token, ownerAccount, arbitraryAccount) {
    ...
    let allowed = await token.allowance(ownerAccount, arbitraryAccount);
    ...
    allowed = await token.allowance(ownerAccount, arbitraryAccount);
    assert.isTrue(web3.utils.toBN(allowed).eq(web3.utils.toBN(1500)));
    ...
    assert.isTrue(web3.utils.toBN(balanceFeeAccount).sub(web3.utils.toBN(initialBalanceFeeAccount)).eq(web3.utils.toBN(feeAmount)));
}
...
async function transferFromWithFees(token, ownerAccount, arbitraryAccount, minter) {
    ...
    allowed = await token.allowance(ownerAccount, arbitraryAccount);
    assert.isTrue(web3.utils.toBN(allowed).eq(web3.utils.toBN(634)));
    ...
    let balance0 = await token.balanceOf(ownerAccount);
    assert.isTrue(web3.utils.toBN(balance0).eq(web3.utils.toBN(900).sub(web3.utils.toBN(534)).sub(web3.utils.toBN(feeAmount))));
    let balance3 = await token.balanceOf(arbitraryAccount);
    assert.isTrue(web3.utils.toBN(balance3).eq(web3.utils.toBN(534)));
    let balanceFeeAccount = await token.balanceOf(feeAccount);
    assert.isTrue(web3.utils.toBN(balanceFeeAccount).sub(web3.utils.toBN(initialBalanceFeeAccount)).eq(web3.utils.toBN(feeAmount)));
}

async function sampleTransferFrom(token, ownerAccount, arbitraryAccount, minter) {
    let allowed = await token.allowance(ownerAccount, arbitraryAccount); // TODO not this
    assert.isTrue(web3.utils.toBN(allowed).eq(web3.utils.toBN(0)));
    await mint(token, ownerAccount, 900, minter); // TODO maybe this
    await token.approve(arbitraryAccount, 634); // TODO not this
    allowed = await token.allowance(ownerAccount, arbitraryAccount); // TODO not this
    assert.isTrue(web3.utils.toBN(allowed).eq(web3.utils.toBN(634)));
    ...
    let balance0 = await token.balanceOf(ownerAccount);
    assert.isTrue(web3.utils.toBN(balance0).eq(web3.utils.toBN(900).sub(web3.utils.toBN(534))));
    let balance3 = await token.balanceOf(arbitraryAccount);
    assert.isTrue(web3.utils.toBN(balance3).eq(web3.utils.toBN(534)));
}

async function increaseAllowance(token, to, amount, from) {
    await token.increaseAllowance(to, amount, { from: from });
}
...
function validateTransferEvent(transferEvent, from, to, amount) {
    ...
    assert.equal(eventResult.args.amount, amount);
}
...
async function customInitializeTokenWithProxy(rawToken, _masterMinter, _pauser, _blacklister, _owner) {
    ...
    await proxiedToken.initialize(name, symbol, decimals, _masterMinter, _pauser, _blacklister, _owner);
...
async function getAdmin(proxy) {
    let adm = web3.utils.toChecksumAddress(await web3.eth.getStorageAt(proxy.address, adminSlot));
    return adm;
}

async function getImplementation(proxy) {
    let impl = web3.utils.toChecksumAddress(await web3.eth.getStorageAt(proxy.address, implSlot));
    return impl;
}

async function getInitializedV1(token) {
    var slot8Data = await web3.eth.getStorageAt(token.address, 6);
    ...
    if (slot8DataLength == 3) {
        ...
        if (slot8DataLength == 46) {
            initialized = "0x" + slot8Data.substring(2,4); // first 2 hex-chars after 0x
            masterMinterStart = 4;
        } else if (slot8DataLength == 42) {
            initialized = "0x00";
            masterMinterStart = 2;
        } else if (slot8DataLength == 44) {
            initialized = "0x01";
            masterMinterStart = 2;
        } else {
        ...
        assert.equal((await token.masterMinter.call()).toLowerCase(), masterMinterAddress);
    }
    return initialized;
}
