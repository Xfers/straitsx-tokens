...
async function run_tests(newToken, accounts) {
  ...
  it('nut003 should fail to upgradeTo to null contract address', async function () {
    ...
    await expectRevert(proxy.upgradeTo("0x0000000000000000000000000000000000000000", {from: proxyOwnerAccount}));
  ...
  it('nut004 should fail to upgradeToAndCall to null contract address', async function () {
    ...
    await expectRevert(proxy.upgradeToAndCall("0x0000000000000000000000000000000000000000", initializeData, { from: proxyOwnerAccount }));
  ...
  it('nut005 should fail to initialize contract twice', async function () {
    await expectRevert(token.initialize(name, symbol, decimals, masterMinterAccount, pauserAccount, blacklisterAccount, tokenOwnerAccount));
    customVars = [];
    await checkVariables([token], [customVars]);
  });

  it('nut006 should fail to call contract function with adminAccount', async function () {
    await expectRevert(token.isMinter(minterAccount, { from: proxyOwnerAccount }));
  ...
  it('nut008 shoud fail to update proxy storage if state-changing function called directly in FiatToken', async function () {
    await rawToken.initialize(name, symbol, decimals, masterMinterAccount, pauserAccount, blacklisterAccount, tokenOwnerAccount);
  ...
  it('nut010 should fail to call updateToAndCall with non-adminAccount', async function () {
   ...
   it('nut011 should fail to upgradeToAndCall with initialize (already set variables)', async function () {
    ...
    var upgradedToken = await UpgradedFiatTokenNewFields.new();
    var data = encodeCall('initialize', ['string','string','uint8','address','address','address','address','bool','address','uint256'], [name, symbol, decimals, masterMinterAccount, pauserAccount, blacklisterAccount, tokenOwnerAccount, true, pauserAccount, 12]);
