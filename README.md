...
|`operatorSend`| `from : ByStr20, to : ByStr20, value : Uint128, initiator : ByStr20` | A transition for default operators to send tokens on behalf of a `from` address.  <br>  :warning: **Note:**   1) Only non-revoked default operators can invoke this transition. 2) Default operators can transfer any amount on behalf of a token holder. 3) The `initiator`, the `from` address and the `recipient` should not be blacklisted.|<h1> :x: </h1>  |
